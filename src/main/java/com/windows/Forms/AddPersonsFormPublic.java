...
public class AddPersonsFormPublic extends AddPersonsForm {
	...
	private String generate_textboxes(){
		...
		name=new TextBox.Builder("name", "Introduceti denumirea (ex. SRL 'Mold')").build().concatenator(new Label("name","Nume"));
		IDNP=new TextBox.Builder("idnp", "Introduceti Codul Fiscal (ex.200288871111)").build().concatenator(new Label("idnp","IDNP"));
		adress1=new TextBox.Builder("adress1", "Introduceti adresa de resedinta").build().concatenator(new Label("adress1","Adresa de Domiciliu"));
